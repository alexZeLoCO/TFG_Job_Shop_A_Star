...
def freeze_data(obj, *, allow=frozenset()):
    ...
    - modules (if allowed) will unchanged. If root object s a module,
      it will be converted to a namedtuple
    ...
    If 'obj' is a function, a ValueError is raised,
    as functions are not read-only
    (e.g. the function's .__globals__ will be accessible).
    However, functions are often useful
    to retain in the data structures,
    so if string 'functions' is found in 'allow',
    functions are *copied* to new functions.
    The user of the data structure *will* be
    allowed access to the original function's .__globals__ attributes etc.,
    so keep this in mind.

    class objects are also not necessarily read-only and
    will by default raise ValueErrors.
    However, if a string 'classes' is in 'allow',
    the class objects will be converted to
    namedtuple instances.

    class instances (beside from built-in immutables like 9, "er", 9.7 etc.)
    will raise ValueError by default.
    If a string 'instances' is set in 'allow', instances will
    be converted to namedtuple instances.

    If other objects than lists, sets,
    instances etc mentioned above are found, a ValueError
    will be raised.

    Notice: The created read-only objects is not hashable if
    the original object contains dicts, as
...
def freeze_data_inner(obj, *, allow=frozenset()):
    ...
    if isinstance(obj, (str, int, float, bytes, type(None), bool)):
        return obj
    # assumed namedtuple
    elif isinstance(obj, tuple) and not isinstance(obj, (Tuple, tuple)):
        return type(obj)(
        ...
        if 'instances' not in allow:
            raise ValueError(
                ("class instances not allowed, %s used. "
...
def freeze_class_or_istance_or_module(obj, allow=frozenset()):
    """
    Convert a class, instanced or module
    to a collections.namedtuple.

    Note: collections.namedtuple does not allow
    single underscore in attributes
    """
    ...
    obj_tuple = namedtuple(obj_name, [
        i for i in dir(obj) if not i.startswith('_')
    ])
    return obj_tuple(
...
def copy_function(obj):
    ...
    Note: functions leak data,
    so a function copy is only a partial copy.
    In particular, avoid resetting dunder-attributes,
    e.g. don't reset .__kwdefaults__
    """
    import functools
