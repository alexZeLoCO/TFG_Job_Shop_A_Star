...
export default makeScene2D(function* (view) {
  ...
  view.add(
    <Title
      ...
      y={0}
      fontSize={100}
      text={"¿Cómo se implementa un heurístico?"}
    />
  );
  ...
  yield* warningBeginSlide(readyRef, "ADD DATA");
...
`,
    ...
    subRef().text("Heurístico Óptimo / Lento (cota inferior)", 0.5),
...
`,
  ...
  yield* warningBeginSlide(readyRef, "CALCULATE MAX");
  ...
  yield view.add(
    <Latex
      ...
      height={0}
      fill={"#00000000"}
      tex={`{\\color{${colors.text}}max(6, 6) = 6}`}
    />
  );
  texRefs[0]().remove();
  texRefs[1]().remove();
